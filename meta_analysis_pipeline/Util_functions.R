...
######################################################################################################
# Taken from the supplementary material of : https://onlinelibrary.wiley.com/doi/10.1002/gepi.22339
######################################################################################################
...
####################################################################################################
# script from https://github.com/ejh243/SCZEWAS/
####################################################################################################
calc_smokescore = function(betas){
  ...
  length(intersect(rownames(betas),cpgs))
  ##############
  ...
  ### filter to overlap with probes on epic array
  ...
  reference_never_median_beta_discovery<-reference_never_median_beta_discovery[which(cpgs %in% rownames(betas))]
  reference_never_median_beta_replication<-reference_never_median_beta_replication[which(cpgs %in% rownames(betas))]
  all_effect<-all_effect[which(cpgs %in% rownames(betas))]
  cpgs<-cpgs[which(cpgs %in% rownames(betas))]
  ...
  #--------------------------------------------------------------------------------------
  ###This is added for a more up to data analysis
  
  #Add the 5 'new'CpG sites of the study of Li et al (2018)
  #For this, use 'never smokers' and 'current smokers'
  
  cpgs2 <- c("cg17287155", "cg09022230","cg10750182","cg23931381","cg03646329")
  
  #Please note, they did not use a reference and replication cohort. Instead, they checked whethet other studies found the same CpG sites
  never_median_M2 <- c(0.86, 0.76, 0.62, 0.89, 0.82)
  ever_median_M2 <- c(0.84, 0.73, 0.60, 0.87, 0.79)
  
  #Note, these are M values, should be converted into beta values
  never_median_beta2 <- 1/(1+(1/(2^never_median_M2)))
  ever_median_beta2 <- 1/(1+(1/(2^ever_median_M2)))
  
  #To calculate the effect size: betasmoker - betanever * 100%
  effect2 <- (ever_median_beta2 - never_median_beta2)*100
  
  lidata <- as.data.frame(cbind(cpgs2, effect2))
  lidata$cpgs2<- as.character(lidata$cpgs2)  
  
  lidata$abseffect2 <- abs(effect2)
  overall_effect2 <- mean(lidata$abseffect2)
  
  weights <- lidata$abseffect2/overall_effect2
  lidata$weights <- weights
  
  #select betas
  select2 <- rownames(betas) %in% lidata$cpgs2
  A2 <- subset(betas, select2 =="TRUE")
  
  #-------------------------------------------------------------------------------
  
  #A_down <- rbind(A_down, A2)
  ...
  identical(rownames(A_up),Illig_data_up$cpgs)
  identical(rownames(A_down),Illig_data_down$cpgs)
  ...
  smokingscore = scores_combined_A
  names(smokingscore) = colnames(betas)
